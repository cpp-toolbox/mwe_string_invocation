...
std::optional<std::string> invoke(std::string &invocation, std::vector<meta_utils::MetaType> available_types) {
    ...
    meta_utils::MetaFunctionSignature mfs_mul_vec("glm::vec3 mul_vec(glm::vec3 v, float c)",
    ...
    meta_utils::MetaFunctionSignature mfs_sum_vec("glm::vec3 sum_vec(std::vector<glm::vec3> vecs)",
                                                  type_name_to_meta_type_map);
    ...
    meta_utils::MetaFunctionSignature mfs_force_to_string("std::string force_to_string(Force f)",
                                                          type_name_to_meta_type_map);
    meta_utils::MetaFunctionSignature mfs_string_to_force("Force string_to_force(std::string s)",
                                                          type_name_to_meta_type_map);
...
std::optional<Force> vec_to_force_string_invoker(std::string &input) {
    std::regex re(
        R"(^\s*vec_to_force\s*\(\s*(\s*\(\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*\)\s*)\s*\)\s*$)");
    std::smatch match;
    if (!std::regex_match(input, match, re))
        return std::nullopt;
...
std::optional<glm::vec3> force_to_vec_string_invoker(std::string &input) {
    std::regex re(
        R"(^\s*force_to_vec\s*\(\s*(\s*\(\s*-?\d+(?:\.\d+)?\s*,\s*\s*\(\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*\)\s*\s*\)\s*)\s*\)\s*$)");
    std::smatch match;
    if (!std::regex_match(input, match, re))
        return std::nullopt;
...
std::optional<std::string> force_to_string_string_invoker(std::string &input) {
    std::regex re(
        R"(^\s*force_to_string\s*\(\s*(\s*\(\s*-?\d+(?:\.\d+)?\s*,\s*\s*\(\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*\)\s*\s*\)\s*)\s*\)\s*$)");
    std::smatch match;
    if (!std::regex_match(input, match, re))
        return std::nullopt;
